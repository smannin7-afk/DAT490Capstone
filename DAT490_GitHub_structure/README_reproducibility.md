...
The notebooks and scripts should be stored in folders, not all at the repository root.
...
├── requirements.txt
├── data/
│   └── prepare_hmda_dataset.py
├── evaluation/
│   └── 01_evaluation_model_comparison.ipynb
├── multilevel/
│   ├── 01_multilevel_analysis.ipynb
│   └── run_multilevel_lme4.R
├── reports/
│   ├── final_draft.pdf
│   └── DAT490_final_report.tex
└── outputs/
    ├── multilevel/
    ├── predictions/
    └── evaluation/
...
Do not commit the raw 2023 HMDA LAR file because it is too large. Place the raw file locally here:
...
`outputs/multilevel/multilevel_model_input.csv` is an intermediate file created by `multilevel/01_multilevel_analysis.ipynb` from the cleaned parquet and state covariates. Do not commit it unless the instructor specifically asks for generated intermediate files.
...
8. Generate or place the held-out prediction file at:

```text
outputs/predictions/heldout_predictions.csv
```

This file is produced by the model-training pipeline. If the training notebook is not included in the repo, include a short note in `models/README.md` explaining how the saved held-out predictions were generated.
